...
function main() {
  ...
  for (var i = 0; i < mesh.indices.length / 3; i++) {
    ...
    var n = vec3.create(); // the norm of the vector
    vec3.cross(n, a, b);
    vec3.normalize(n, n);
    ...
    for (var j = 0; j < 3; j++) {
      ...
      for (var k = 0; k < 3; k++) {
        mesh.vertexNormals[3 * vIndex + k] += n[k] * Math.acos(vec3.dot(a, b)); // Note: weighting by the angle is more accurate, but not necessary in this example (negligible effect), we are doing it for every component
      }
    }
  }
