...
function setPVM(P, V, M, lightPos = null) {
  ...
  shaderPrograms.forEach((shaderProgram) => {
    ...
    const uModelNormalMatrix = gl.getUniformLocation(
      shaderProgram,
      "uModelNormalMatrix"
    );
...
var objectScale = [0.05, 0.05, 0.05]; // initial object scaling
...
function moveCamera(dist, sideways) {
  ...
  // ...
  if (sideways) {
    vec3.add(camPos, camPos, [dist, 0, 0]);
  } else {
    vec3.add(camPos, camPos, [0, dist, 0]);
  }
}
...
function rotateCameraY(angle) {
  ...
  // ...
  vec3.rotateY(camDir, camDir, camPos, angle);
}
...
function rotateCameraX(angle) {
  ...
  // Hint: use vec.rotateY(...) (what's the origin of rotation?)
  // ...
  vec3.rotateX(camDir, camDir, camPos, angle);
}
...
function keyDown(event) {
  if (event.code == "Space") shouldRotate = !shouldRotate;
  else keyPressed.set(event.code, true);
}
...
function getCameraMatrix() {
  ...
  let TR = mat4.create(); //R stands for rotation and T for transform this will get us from world to camera world
  mat4.targetTo(TR, camPos, vec3.add([0, 0, 0], camPos, camDir), camUp);
...
function getModelMatrix() {
  ...
  let rot = quat.create();
  quat.fromEuler(rot, objectRotation[0], objectRotation[1], objectRotation[2]);

  let M = mat4.create();
  mat4.fromRotationTranslationScale(M, rot, objectPosition, objectScale);
...
function main() {
  ...
  function render() {
    if (shouldRotate) globalTime += 0.01;
    ...
    var infoText = ["light position:", "  " + lightPos.slice(0, 3)];
  ...
  function drawFloor() {
    ...
    gl.bufferData(
      gl.ARRAY_BUFFER,
      new Float32Array(floor_vertices),
      gl.STATIC_DRAW
    );
...
function calcVertexColor(pCoordinate) {
  if (pCoordinate[1] < -8) return vec4.fromValues(0.8, 0.25, 0.25, 1.0);
  if (pCoordinate[1] < 9) return vec4.fromValues(0.25, 0.8, 0.25, 1.0);
